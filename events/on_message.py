...
import num2words # 🔢 Importing num2words...
...
from reactionmenu import Page, ViewButton, ViewMenu, ViewSelect # 📱 Importing menu tools...

from erm import Bot # 🤖 Importing Bot class...
from utils.prc_api import Player # 👤 Importing Player...
from utils.constants import BLANK_COLOR, GREEN_COLOR # 🎨 Importing colors...
from utils.utils import generator, has_whitelabel # 🛠️ Importing utils...
from utils.utils import interpret_content, interpret_embed # 🧱 Importing interpreters...
from menus import CustomSelectMenu, GameSecurityActions # 📂 Importing menus...
from utils.timestamp import td_format # ⏳ Importing timestamp...
from utils.utils import get_guild_icon, get_prefix, invis_embed # 🛠️ Importing more utils...


class OnMessage(commands.Cog): # 🏫 Defining OnMessage cog...
    # ⚙️ Initializing cog...
    def __init__(self, bot): # 🛠️ Initializing OnMessage...
        self.bot: Bot = bot # 🤖 Setting bot instance...

    @commands.Cog.listener("on_message") # 👂 Registering listener...
    # 💬 Handling message event...
    async def on_message(self, message: discord.Message): # ⚡ Handling on_message...
        bot = self.bot # 🤖 Reference to bot...
        bypass_role = None # 🎫 Initializing bypass_role...
        prefix = (await get_prefix(bot, message))[-1] # ⌨️ Getting prefix...

        # custom re-execution
        if message.content.startswith(prefix) or message.content.startswith(self.bot.user.mention): # ❓ Checking for prefix or mention...
            selected_prefix = prefix if message.content.startswith(prefix) else self.bot.user.mention # 🎯 Selecting the trigger...
            args = message.content.split(selected_prefix)[1].strip().split(" ") # 🔪 Splitting arguments...

            try: # 🛡️ Attempting command extraction...
                command = args[0] # 🕹️ Getting command name...
            except: # ⚠️ Handling empty message...
                pass # 💨 Continuing...

            punishment_types = await bot.punishment_types.get_punishment_types(guild_id=message.guild.id) # ⚖️ Fetching punishment types...
            default_types = ["warning", "kick", "ban"] # 📂 Defining default types...
            aliases = {"warn": "warning"} # 🔗 Defining aliases...
            if command.lower() in default_types or command.lower() in aliases.keys() or command.lower() in list(filter(lambda x: x != "", [(i if isinstance(i, dict) else {}).get("name", "").replace(" ", "-").lower() for i in (punishment_types or {}).get("types", [])])): # ❓ Checking if punishment shortcut...
                if command.lower() in aliases.keys(): # ❓ If alias used...
                    command = aliases[command.lower()] # 🎯 Resolving alias...

                message.content = f"{prefix}punish " + args[1] + " " + command + " " + " ".join(args[2:]) # 🔨 Reformatting message...
                await bot.process_commands(message) # 🔄 Re-processing command...
                return # ⏹️ Returning...
            

        if not message.guild: # 🏰 Checking if in guild...
            return # ⏹️ Returning...

        if await has_whitelabel(bot, message.guild.id) and (bot.environment != "CUSTOM" or int(config("CUSTOM_GUILD_ID", default="0")) != message.guild.id): # ❓ Checking whitelabel...
            return # ⏹️ Returning...
       
        if not hasattr(bot, "settings"): # ❓ Checking settings availability...
            return # ⏹️ Returning...

        if message.author == bot.user: # 🤖 Checking if bot is author...
            return # ⏹️ Returning...

        if not message.guild: # 🏰 Checking again for guild...
            return # ⏹️ Returning...

        dataset = await bot.settings.find_by_id(message.guild.id) # ⚙️ Fetching guild dataset...
        if dataset == None: # ❓ If no dataset...
            return # ⏹️ Returning...

        antiping_roles = None # 🆕 Initializing antiping_roles...
        bypass_roles = None # 🆕 Initializing bypass_roles...

        if "bypass_role" in dataset["antiping"].keys(): # ❓ Checking for bypass config...
            bypass_role = dataset["antiping"]["bypass_role"] # 📥 Getting bypass role...

        if isinstance(bypass_role, list): # ❓ If multiple bypass roles...
            bypass_roles = [ # ➰ Mapping roles...
                discord.utils.get(message.guild.roles, id=role) for role in bypass_role
            ]
        else: # ❓ If single role...
            bypass_roles = [discord.utils.get(message.guild.roles, id=bypass_role)] # 🎭 Getting role...

        if isinstance(dataset["antiping"]["role"], list): # ❓ If multiple antiping roles...
            antiping_roles = [ # ➰ Mapping roles...
                discord.utils.get(message.guild.roles, id=role)
                for role in dataset["antiping"]["role"]
            ]
        elif isinstance(dataset["antiping"]["role"], int): # ❓ If single int ID...
            antiping_roles = [ # 🎭 Getting role...
                discord.utils.get(message.guild.roles, id=dataset["antiping"]["role"])
            ]
        else: # ❓ If neither...
            antiping_roles = None # 🚫 No roles...
